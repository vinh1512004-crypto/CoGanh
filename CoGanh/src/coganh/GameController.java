...
public class GameController {
    ...
    public volatile boolean botThinking = false; // Ngăn click chuột khi bot đang suy nghĩ
    ...
    public int[][][] board = {
            ...
            { { 12, 136, -1 }, { 136, 136, 0 }, { 264, 136, 0 }, { 390, 136, 0 }, { 516, 136, -1 } },
    ...
    class GameStateSnapshot {
        ...
        int cODaChon, cOBatBuoc, cConsecutiveWins;
        ...
        public GameStateSnapshot() {
            ...
            cODaChon = oDaChon;
            cOBatBuoc = oBatBuoc;
            cConsecutiveWins = consecutiveWins;
        ...
        public void restore() {
            ...
            oDaChon = cODaChon;
            oBatBuoc = cOBatBuoc;
            consecutiveWins = cConsecutiveWins;
    ...
    public void choiLaiVanMoi() {
        ...
        listBiMo.clear();
        khoiTao();
        dongBoQuanCo(); // Đảm bảo quanDo/quanXanh đồng bộ chính xác với board
        gp.veLaiToanBo();
    }
            ...
            public void actionPerformed(java.awt.event.ActionEvent e) {
                ...
                if (chonBlue) {
                    ...
                    if (timeLeftBlue <= 0) {
                        ...
                        timeoutEnd = true;
                        countdownTimer.stop();
                        sm.addResult("DO", isPvE, playerNameRed); // Đỏ thắng khi Xanh hết giờ
                    ...
                    if (timeLeftRed <= 0) {
                        ...
                        timeoutEnd = true;
                        countdownTimer.stop();
                        sm.addResult("XANH", isPvE, playerNameBlue); // Xanh thắng khi Đỏ hết giờ
                        deleteCurrentSaveFile();
                    }
                }
                gp.veLaiToanBo();
    ...
    public BanCo createBanCoAo() {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                int v = board[i][j][2];
                // Chuẩn hóa trạng thái UI về giá trị logic cho AI
                if      (v == 2)  v = 1;              // xanh đang chọn → xanh
                else if (v == -2) v = -1;             // đỏ đang chọn  → đỏ
                else if (v == 3)  v = 0;              // ô gợi ý        → trống
                else if (v == 4)  v = (chonBlue ? 1 : -1); // quân bị vây → về phe gốc
                banCoAo.maTran[i][j] = v;
            }
        }
        return banCoAo;
    }
    ...
    void xuLyChuot(int x_in, int y_in) {
        if (isAnimating || botThinking)
            return; // Chặn click khi đang animation hoặc bot đang suy nghĩ
        ...
        if (!moCo) {
            if ((chonBlue && o == 1) || (!chonBlue && o == -1)) {
                if (!chon) {
                    ...
                    // Gán lại trị quân cờ 1 lần duy nhất, bên ngoài vòng lặp
                    board[oCu_i][oCu_j][2] = chonBlue ? 1 : -1;
                    chon = false;
                    if (chuMo) {
                        listBiMo.clear();
                        listMo.clear();
    ...
    void ktraGanh(int i, int j, int o) {
        // Gánh ngang: quân trái-phải
        if (ktra(j + 1) && ktra(j - 1)) {
            if (!xuLyCapGanh(i, j + 1, i, j - 1, o))
                return;
        }
        // Gánh dọc: quân trên-dưới
        if (ktra(i + 1) && ktra(i - 1)) {
            if (!xuLyCapGanh(i + 1, j, i - 1, j, o))
                return;
        }
        // Gánh chéo: chỉ áp dụng với ô chẵn (i+j) % 2 == 0, kiểm tra độc lập
        if ((i + j) % 2 == 0 && ktra(i + 1) && ktra(i - 1) && ktra(j + 1) && ktra(j - 1)) {
            for (int b = -1; b < 2; b += 2) {
        ...
        if (isPvE && end == 0) {
            if (chonBlue && chuMo) {
                new Thread(() -> {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        if (!gp.start || end != 0) return; // Guard: thoát game giữa chừng
                        moCo = true;
                        setMoCo();
                        if (end == 0 && chonBlue)
                            historyStack.add(new GameStateSnapshot());
                ...
                new Thread(() -> {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        if (!gp.start || end != 0) { botThinking = false; return; } // Guard
                        moCo = true;
                        setMoCo();
                    });
                    try {
