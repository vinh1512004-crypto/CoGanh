...
public class BanCo {
    ...
    public BanCo copyNhap() {
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                banCoMoi.maTran[i][j] = this.maTran[i][j];
            }
        }
                    ...
                    for (int a = -1; a <= 1; a++) {
                        for (int b = -1; b <= 1; b++) {
                            ...
                            if (toaDoHopLe(ni) && toaDoHopLe(nj)) {
                                ...
                                if (maTran[ni][nj] == 0 || maTran[ni][nj] == 3) {
    ...
    private void kiemTraThietLapMo(int r, int c, int phe) {
        boolean taoTheMo = false;
        
        if (toaDoHopLe(c - 1) && toaDoHopLe(c + 1) && maTran[r][c - 1] == phe && maTran[r][c + 1] == phe) taoTheMo = true;
        if (toaDoHopLe(r - 1) && toaDoHopLe(r + 1) && maTran[r - 1][c] == phe && maTran[r + 1][c] == phe) taoTheMo = true;
        if ((r + c) % 2 == 0) {
            if (toaDoHopLe(r - 1) && toaDoHopLe(c - 1) && toaDoHopLe(r + 1) && toaDoHopLe(c + 1) && maTran[r - 1][c - 1] == phe && maTran[r + 1][c + 1] == phe) taoTheMo = true;
            if (toaDoHopLe(r - 1) && toaDoHopLe(c + 1) && toaDoHopLe(r + 1) && toaDoHopLe(c - 1) && maTran[r - 1][c + 1] == phe && maTran[r + 1][c - 1] == phe) taoTheMo = true;
        }
    ...
    private void xulyGanh(int r, int c, int phe) {
        int doiPhuong = -phe;
        
        if (toaDoHopLe(c - 1) && toaDoHopLe(c + 1)) {
            if (maTran[r][c - 1] == doiPhuong && maTran[r][c + 1] == doiPhuong) {
                maTran[r][c - 1] = phe; maTran[r][c + 1] = phe;
            }
        }
        if (toaDoHopLe(r - 1) && toaDoHopLe(r + 1)) {
            if (maTran[r - 1][c] == doiPhuong && maTran[r + 1][c] == doiPhuong) {
                maTran[r - 1][c] = phe; maTran[r + 1][c] = phe;
            }
        }
        if ((r + c) % 2 == 0) {
            if (toaDoHopLe(r - 1) && toaDoHopLe(c - 1) && toaDoHopLe(r + 1) && toaDoHopLe(c + 1)) {
                if (maTran[r - 1][c - 1] == doiPhuong && maTran[r + 1][c + 1] == doiPhuong) {
            ...
            for (int b = -1; b <= 1; b++) {
                ...
                if (toaDoHopLe(nr) && toaDoHopLe(nc)) {
                    ...
                    if (maTran[nr][nc] == 0) {
                        ...
                        if (checkDuongTho(nr, nc, phe, daDuyet, khoi)) coTho = true;
                    }
                }
            }
        }
    ...
    public int chamDiem() {
        ...
        // Bot (-1) muốn Xanh (1) bị dồn vào góc -> Điểm vị trí Xanh CÀNG THẤP CÀNG TỐT cho Bot.
        int[][] viTri = {
            { 0,  1,  2,  1,  0 },
            { 1,  3,  4,  3,  1 },
            { 2,  4,  6,  4,  2 },
