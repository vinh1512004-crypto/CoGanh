...
public class GamePanel_CoGanh extends JPanel {
    ...
    private void loadImage() {
        try {
            back = ImageIO.read(getClass().getResourceAsStream("HinhAnh/back.png"));
            background = ImageIO.read(getClass().getResourceAsStream("HinhAnh/br_coganh.png"));
            boardImg = ImageIO.read(getClass().getResourceAsStream("HinhAnh/board.png"));
            backgr = ImageIO.read(getClass().getResourceAsStream("HinhAnh/brg.png"));
            on = ImageIO.read(getClass().getResourceAsStream("HinhAnh/on.png"));
            off = ImageIO.read(getClass().getResourceAsStream("HinhAnh/off.png"));
            hd[0] = ImageIO.read(getClass().getResourceAsStream("HinhAnh/hd1.jpg"));
            hd[1] = ImageIO.read(getClass().getResourceAsStream("HinhAnh/hd2.jpg"));
            hd[2] = ImageIO.read(getClass().getResourceAsStream("HinhAnh/hd3.jpg"));
            left = ImageIO.read(getClass().getResourceAsStream("HinhAnh/left.png"));
            right = ImageIO.read(getClass().getResourceAsStream("HinhAnh/right.png"));
            undoIcon = ImageIO.read(getClass().getResourceAsStream("HinhAnh/QL.png"));
            logoImg = ImageIO.read(getClass().getResourceAsStream("HinhAnh/Logo.png"));
            hoTroIcon = ImageIO.read(getClass().getResourceAsStream("HinhAnh/hotro.png"));
            try {
                pieceRed = ImageIO.read(getClass().getResourceAsStream("HinhAnh/Q_Do.png"));
            } catch (Exception ex) {
                System.out.println("Không tìm thấy Q_Do.png, dùng màu mặc định.");
            }
            try {
                pieceBlue = ImageIO.read(getClass().getResourceAsStream("HinhAnh/Q_Xanh.png"));
            } catch (Exception ex) {
            ...
            try {
                kChuaImg = ImageIO.read(getClass().getResourceAsStream("HinhAnh/K_chua.png"));
            } catch (Exception ex) {
            ...
            try {
                ...
                for (int i = 0; i < 6; i++) {
                    BufferedImage raw = ImageIO.read(getClass().getResourceAsStream("HinhAnh/" + avatarNames[i]));
                    if (raw != null) {
                ...
                for (int j = 0; j < 5; j++) {
                    if (gc.board[i][j][2] != 0) {
                        ...
                        if (gc.board[i][j][2] == -1) {
                            ...
                            // Quân bị bôi đen (bắt buộc vào gánh): vẽ đúng kích thước như quân thường
                            // rồi phủ lớp màu đen bán trong suốt lên trên để báo hiệu bị ràng buộc
                            BufferedImage forcedImg = gc.chonBlue ? pieceBlue : pieceRed;
                            if (forcedImg != null)
                                g.drawImage(forcedImg, px, py, psize, psize, null);
                            else {
                                g.setColor(gc.chonBlue ? Color.blue : Color.red);
                                g.fillOval(px, py, psize, psize);
                            }
                            // Lớp phủ đen hoàn toàn để phân biệt rõ với quân thường
